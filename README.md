...
- 有效根必须是同一个父物体的直接子级。
- 按 AAO 规则比较实际生效值；忽略基础值为零时无效的曲线、重力为零时无效的 Gravity Falloff、等价的权限存储方式和碰撞器列表顺序。
- `Multi Child Type` 必须为 `Ignore`。
- 不合并重叠根、Avatar 外部根、禁用的 PhysBone、动画控制的 PhysBone 或有效根、Humanoid 名称依赖路径以及带 Unity/VRC Constraint 的链。
